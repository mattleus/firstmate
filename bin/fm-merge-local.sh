...
# project's default branch to the crewmate's ml/<id> branch (falling back to
# fm/<id> for in-flight pre-cutover tasks).
#
...
if ! git -C "$PROJ" rev-parse --verify --quiet "refs/heads/$BRANCH" >/dev/null; then
  BRANCH="fm/$ID"
  git -C "$PROJ" rev-parse --verify --quiet "refs/heads/$BRANCH" >/dev/null || { echo "error: neither branch ml/$ID nor fm/$ID exists in $PROJ" >&2; exit 1; }
fi
