...
Ship briefs also tell the crewmate to verify `pwd -P` and `git rev-parse --show-toplevel` before creating `ml/<id>`, then stop with a blocked status if it landed in the primary checkout.
